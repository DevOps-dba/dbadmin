$(document).ready(function() {
        ...
        var user_priv_keyword = sessionStorage.getItem('user_priv_keyword');
        var user_priv_current_page = sessionStorage.getItem('user_priv_current_page');
        var user_priv_table_length = sessionStorage.getItem('user_priv_table_length');

        var data =  {
                "page":1,
                "user_priv_keyword":'',
                "user_priv_table_length":10
            };

        if ( user_priv_keyword ) {
            $('#user_priv_keyword').val(user_priv_keyword);
            data["user_priv_keyword"] = user_priv_keyword;
        }
        if ( user_priv_current_page ) {
            data["page"] = user_priv_current_page;
        }
        if ( user_priv_table_length ) {
            $('select#user_priv_table_length').val(user_priv_table_length);
            data["user_priv_table_length"] = user_priv_table_length;
        }

        // 点击提交表单，重新设置sessionStorage的值，并且重新初始化ajax的data数据
        $('#search_btn').click(function () {
            var user_priv_keyword = $('input#user_priv_keyword').val();
            var user_priv_table_length = $('select#user_priv_table_length').val();

            data = {
                "page":1,
                "user_priv_keyword":user_priv_keyword,
                "user_priv_table_length":user_priv_table_length
            };
            if ( user_priv_keyword ) {
                sessionStorage.setItem('user_priv_keyword',user_priv_keyword );
            }
            else {
                sessionStorage.removeItem('user_priv_keyword');
            }
            sessionStorage.setItem('user_priv_current_page',1 );
            sessionStorage.setItem('user_priv_table_length',user_priv_table_length );

            $.ajax({
                url:'/users/user_priv_list_ajax',
                type:'POST',
        ...
        $.ajax({
                ...
                success:function (callback) {
                    if (callback.page_content) {
                        ...
                        var page_content=callback.page_content;
                        var per_page = callback.per_page;
                        $('tbody#user_priv_dynamic').append(page_content);
                        var notes = "第1页/共 " + Math.ceil(page_count/per_page) + "页, 每页" + per_page + "条/共" + page_count + " 条";
                        $('div#user_priv_notes').text(notes);
                    } else {
                        $('tbody#user_priv_dynamic').empty();
    ...
    function monitor(totalpages,current_page,data) {
        $('#user_priv_pageLimit').bootstrapPaginator({
            ...
            itemTexts: function (type, page, current) {
                ...
                onPageClicked: function (event, originalEvent, type, page){     //给每个页眉绑定一个事件，其实就是ajax请求，其中page变量为当前点击的页上的数字。
                    sessionStorage.setItem('user_priv_current_page',page );
                    data["page"] = page;
                    $.ajax({
                        url:'/users/user_priv_list_ajax',
                        type:'POST',
    ...
    function checkbox_op() {
        ...
        //     var new_href_string;
        //
        //     $("table#user_priv_table tr.gradeX td:nth-child(1)").each(function (key, value) {
 ...
 * Last edited by qiankun on 2018/3/28.
 */
