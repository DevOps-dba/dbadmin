$(document).ready(function() {
        ...
        var promlist_service_name = sessionStorage.getItem('promlist_service_name');
        var promlist_item_name = sessionStorage.getItem('promlist_item_name');
        var promlist_datetime = sessionStorage.getItem('promlist_datetime');
        var promlist_current_page = sessionStorage.getItem('promlist_current_page');
        var promlist_data_length = sessionStorage.getItem('promlist_data_length');

        var data =  {
            "page":1,
            "promlist_service_name":'',
            "promlist_item_name":'',
            "promlist_datetime":'',
            "promlist_data_length":20
        };

        if ( promlist_service_name ) {
            $('#promlist_service_name').val(promlist_service_name);
            data["promlist_service_name"] = promlist_service_name;
        }
        if ( promlist_item_name ) {
            $('#promlist_item_name').val(promlist_item_name);
            data["promlist_item_name"] = promlist_item_name;
        }
        if ( promlist_datetime ) {
            $('#promlist_datetime').val(promlist_datetime);
            data["promlist_datetime"] = promlist_datetime;
        }
        if ( promlist_current_page ) {
            data["page"] = promlist_current_page;
        }
        if ( promlist_data_length ) {
            $('select#promlist_data_length').val(promlist_data_length);
            data["promlist_data_length"] = promlist_data_length;
        }

        // 点击提交表单，重新设置sessionStorage的值，并且重新初始化ajax的data数据
        $('#search_btn').click(function () {
            var promlist_service_name = $('input#promlist_service_name').val();
            var promlist_item_name = $('input#promlist_item_name').val();
