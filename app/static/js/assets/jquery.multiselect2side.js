...
(function($)
{
	jQuery.fn.multiselect2side = function (o) {
		...
		return this.each(function () {
			var	el = $(this);
			var selected_list = [];
			var multi_select_base = $('input#multi_select_base');
			...
			leftSel.dblclick(function () {
				$(this).find("option:selected").each(function(i, selected){
					...
					if (o.maxSelected < 0 || rightSel.children().size() < o.maxSelected) {
						...
						selected_list.push(selected.text);
						el.find("[value=" + "'" + $(selected).val() + "'" + "]").attr("selected", true).remove().appendTo(el);
					}
				});
				if (multi_select_base.length > 0) {
					multi_select_base.val(selected_list);
				}
				$(this).trigger('change');
			});
			...
			rightSel.dblclick(function () {
				$(this).find("option:selected").each(function(i, selected){
					...
					selected_list.pop(selected.text);
